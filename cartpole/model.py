...
def simulate_dimensionless_version(
    cartpole_params: CartPoleParams,
) -> tuple[AcadosSim, np.array]:
    sim = AcadosSim()
    ...
    # a0 = np.array([0.0])
    a0 = (
        0.1 * cartpole_params.Fmax.item() / (cartpole_params.M.item() * g)
    )  # scaled force
...
if __name__ == "__main__":
    ...
    # large cartpole
    large_cartpole_sim, large_s_log = simulate_dimensionless_version(
    ...
    # small cartpole
    small_cartpole_sim, small_s_log = simulate_dimensionless_version(
    ...
    dt = 0.5 * (
        small_cartpole_sim.solver_options.T + large_cartpole_sim.solver_options.T
    )
