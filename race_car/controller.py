...
from race_car.utils.config import CarParams, create_acados_params, get_default_car_params
from leap_c.ocp.acados.parameters import AcadosParameterManager
from race_car.acados_ocp import export_ocp
...
class RaceCarControllerConfig:
    """Unused, here just to align with leap-c structure."""

    N_horizon: int = 50
    T_horizon: float = 1.0

    # Extra keyword-only arguments (placeholders)
    car_params: CarParams = field(default_factory=get_default_car_params)
    dimensionless: bool = False
    mpc_car_params: CarParams = field(default_factory=get_default_car_params)


class RaceCarController(AcadosController):
    """TODO: write me"""
    
    cfg: RaceCarControllerConfig

    def __init__(self, cfg: RaceCarControllerConfig | None = None, car_params: CarParams = get_default_car_params(), dimensionless: bool = False):
    ...
    def forward(
        ...
        # TODO: handle obs extension properly
        ctx, u0, x, u, value = self.diff_mpc(
            x0=torch.cat([obs, torch.zeros(1, 2)], dim=1), p_global=param, p_stagewise=p_stagewise, ctx=ctx
        )
        return ctx, u0
...
def export_parametric_ocp(car_params: CarParams, dimensionless: bool, param_manager: AcadosParameterManager) -> AcadosOcp:
    ...
    W_0 = W_0_sqrt @ W_0_sqrt.T
    W = W_sqrt @ W_sqrt.T
    W_e = W_e_sqrt @ W_e_sqrt.T

    # use the external cost module
    ocp.cost.cost_type_0 = ocp.cost.cost_type = ocp.cost.cost_type_e = "EXTERNAL"

    ocp.model.cost_expr_ext_cost_0 = ocp.model.u.T @ W_0 @ ocp.model.u
    ocp.model.cost_expr_ext_cost = (
        W[0,0] * (sref - ocp.model.x[0])**2
        + ca.vertcat(ocp.model.x[1:], ocp.model.u).T @ W[1:, 1:] @ ca.vertcat(ocp.model.x[1:], ocp.model.u)
    )
    ocp.model.cost_expr_ext_cost_e = (
        W_e[0,0] * (sref - ocp.model.x[0])**2
        + ocp.model.x[1:].T @ W_e[1:, 1:] @ ocp.model.x[1:]
    )

    # keep the Gauss-Newton Hessian approximation
    # TODO: continue this
