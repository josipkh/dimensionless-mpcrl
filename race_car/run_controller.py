"""Run the controller and environment in leap-c with default parameters (no learning)."""
...
# extend the Python path to use leap-c
import sys
sys.path.append(str(Path(__file__).resolve().parents[0]))
from race_car import create_controller, create_env
...
def run_controller(
    ...
    trainer = ControllerTrainer(
        val_env=create_env(
            ...
            mpc_car_params=mpc_car_params
        ),
        controller=create_controller(
            controller_name=cfg.controller,
            reuse_code_base_dir=reuse_code_dir,
            car_params=mpc_car_params,
            dimensionless=dimensionless
        ),
...
if __name__ == "__main__":
    ...
    run_controller(cfg, output_path, device=args.device, reuse_code_dir=reuse_code_dir,
                   car_params=car_params, dimensionless=dimensionless, mpc_car_params=mpc_car_params)
